...
(function (global) {
  ...
  const TERMINAL_INSTRUCTIONS = [
    ...
    "",
    "Command fidelity (critical):",
    "- The user's message is the EXACT command string. Pass it to openrouter:shell VERBATIM, unchanged.",
    "- Put the user's command as ONE single entry in the commands array. Do NOT split it on semicolons, pipes, newlines, or any other delimiter. Do NOT break it into multiple commands array entries.",
    "- Do NOT rewrite, reconstruct, re-quote, re-escape, reformat, simplify, expand, paraphrase, or alter the command. Do NOT add or remove quotes, backslashes, semicolons, pipes, variable references, or command substitutions.",
    "- Preserve every character exactly: quotes, semicolons, pipes, backslashes, command substitutions, newlines, variables, and all shell syntax. The shell, not you, parses and runs the command.",
    "- If the command spans multiple lines, send the whole multi-line string as one commands entry, preserving the newlines.",
    "- Never reconstruct a command from your own text or split a compound command into separate fake commands. One user command == exactly one commands array entry, byte-for-byte identical to what the user typed.",
    "",
    "Single-command rule (critical):",
    "- Do ONLY the work needed for the one command the user entered.",
    "- Issue exactly ONE openrouter:shell call containing the user's exact command. Do not issue a second shell call.",
